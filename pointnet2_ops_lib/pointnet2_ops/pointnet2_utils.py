...
class QueryAndGroup(nn.Module):
    ...
    def forward(self, xyz, new_xyz, features=None, subset=True, record_neighbor_stats=False, 
                        return_counts=False, length=None):
        ...
        if self.neighbor_def == 'radius':
            if not length is None:
                raise Exception('radius neighbor definition has not supported point clouds with different lengths')
        ...
        # idx are initialized as zeros, if new_xyz have no neighbors in xyz, idx will be kept 0 as default
            ...
            _, idx, _ = knn.knn_points(new_xyz, xyz, K=num_neighbors, lengths2=length)
            # length specifies how many valid points are actaully contained in xyz
            # knn neighbors only find nn points in these valid points
            idx = idx.int() # idx is of shape (B, npoint, K)
            B, npoint, K = idx.size()
            counts = torch.ones(B, npoint, device=new_xyz.device) * K
            if not length is None:
                # some times K will be larger than length, we need to set counts manually in this case
                counts = torch.minimum(counts, length.unsqueeze(1))
        else:
        ...
        if features is not None:
            ...
            if self.use_xyz:
                ...
                # (B, C + 3, npoint, nsample) or (B, C + 6, npoint, nsample) or (B, C + 9, npoint, nsample)
            else:
...
def group_knn(x, y, features_at_y, K, transpose=False):
    ...
    # nn_relative_position is of shape (B,N1,K,3), relative positions of points in y
